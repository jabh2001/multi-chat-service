import useSeparatedConversations, { useInnerConversationStore } from "../../../hooks/useSeparatedConversations";
    ...
    return (
      <section className="flex flex-col flex-none overflow-auto w-24  group lg:max-w-sm md:w-2/5 transition-all duration-300 ease-in-out max-h-screen">
                ...
                <div className="relative">
                    <label>
                        <input className="rounded-full py-2 pr-6 pl-10 w-full border border-gray-800 focus:border-gray-700 bg-gray-800 focus:bg-gray-900 focus:outline-none text-gray-200 focus:shadow-md transition duration-300 ease-in"
                              type="text" value={search} onChange={evt => setSearch(evt.target.value)} placeholder="Search Messenger"/>
        ...
        <div className="contacts p-2 flex-1 overflow-y-scroll">
            {
                allTabs.map(({ name, conversations }, i) => i === tab ? <ChatsDiv key={`chats_${name}`} conversations={conversations} prefix={name} /> : null)
            }
        </div>
    </section>
    )
              ...
              {tabs.map(({name, count}, i) => (
                <a
                  ...
                  className={classNames(
                    ...
                    'flex whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium flex-1 justify-center items-center'
                  )}
