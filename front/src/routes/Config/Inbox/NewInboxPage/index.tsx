...
import { ChannelTelegramView, ChannelView, ChooseChannelView, FinishView } from "./views"
...
export  function NewInboxTelegramPage({ channel }:Props){
    const addInbox = useInboxStore(state => state.addInbox)
    const navigate = useNavigate()
    const [ step, setStep] = useState(channel? 2 : 1)
    const nextStep = () => setStep(step => step+1)
    ...
    const handleTokenSubmit = async (name:string,token:string) => {
        if(channel){
            console.log(channel)
            const inbox = await postInbox({name, channelType:channel?.replace("-", "").toString(), token})
            addInbox(inbox)
        }
    }
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.tabs}>
                { step > 1 && step < 4 && <button className={styles.prevButton} onClick={prevStep}>PREV</button> }
                { step == 1 && <ChooseChannelView /> }
                { step == 2 && <ChannelTelegramView nextStep={nextStep} handleSubmit={handleTokenSubmit} /> }
                { step == 3 && <FinishView />    }
            </div>
        </div>
    )
