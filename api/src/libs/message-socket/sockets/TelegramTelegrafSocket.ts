...
export class TelegramTelegrafSocket extends Socket {
    ...
    static saveToken(folder:string, token:string){
        ...
        fs.writeFileSync(`${tokenFolder}/${folder}.json`, JSON.stringify({ token }, null, 4), "utf8")
    ...
    static getToken( folder:string){
        ...
        if(!fs.existsSync(`${tokenFolder}/${folder}.json`)){
            return undefined
        }
        const data = fs.readFileSync(`${tokenFolder}/${folder}.json`, "utf8")
